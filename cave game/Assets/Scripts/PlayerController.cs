...
public class PlayerController : MonoBehaviour
{
	...
	public void Update()
	{
		if (PauseMenu.pauseMenu.paused || GetComponent<PlayerIO>().inventory.activeSelf || controlsEnabled == false)
		{
			Cursor.visible = true;
	...
	void FixedUpdate()
	{
		if (PauseMenu.pauseMenu.paused || GetComponent<PlayerIO>().inventory.activeSelf) return;

		float inputX = 0;
		float inputY = 0;
		float inputModifyFactor = 0;
		if (controlsEnabled)
		{
			inputX = Input.GetAxis("Horizontal");
			inputY = Input.GetAxis("Vertical");

			inputModifyFactor = (inputX != 0.0f && inputY != 0.0f && m_LimitDiagonalSpeed) ? .7071f : 1.0f;
		}
